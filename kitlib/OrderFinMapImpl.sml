...
signature ORDER_FINMAP_IMPL = sig
  include MONO_FINMAP
  structure Impl : sig
    datatype bal = L | B | R 
...
functor OrderFinMapImpl (Order : ORDER) : ORDER_FINMAP_IMPL where type dom = Order.t =
  struct
    fun die s = raise Fail ("OrderFinMap." ^ s)

    type dom = Order.t

    fun a < b = case Order.compare (a, b) of
                    LESS => true
                  | _ => false
    val lt = op <
